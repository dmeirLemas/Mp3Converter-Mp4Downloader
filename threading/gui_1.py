...
class YouTubeDownloaderGUI(QMainWindow):
	def __init__(self):
		...
		self.out_location_label = QLabel('Choose output location')
		self.output_button = QPushButton('Click')
		self.output_button.clicked.connect(self.output)
		self.path_layout.addWidget(self.out_location_label)
		self.path_layout.addWidget(self.output_button)
		self.group3.setLayout(self.path_layout)
	...
	def output(self):
		self.dir_name = QFileDialog.getExistingDirectory(self, "Select a Directory")
		if self.dir_name:
			self.path = Path(self.dir_name)
			self.out_location_label.setText(str(self.path))
